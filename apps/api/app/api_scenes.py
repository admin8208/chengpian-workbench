from app.application.scenes.service import *  # noqa: F401,F403
...
def patch_scene_api(scene_id: int, body: ScenePatchIn) -> SceneOut:
    with session_scope() as session:
        s = session.exec(select(Scene).where(Scene.id == scene_id)).first()
        if not s:
...
def bind_scene_asset_api(scene_id: int, body: SceneBindAssetIn) -> SceneOut:
    with session_scope() as session:
        ...
        if not s:
            raise HTTPException(status_code=404, detail="镜头不存在")
        a = session.exec(select(Asset).where(Asset.id == int(body.asset_id))).first()
        if not a:
